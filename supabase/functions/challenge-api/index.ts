  ...
  try {
    ...
    if (action === "start") {
      await supabase.from("challenge_progress").update({ status: "started", started_at: new Date().toISOString() })
        .eq("id", body.progressId).eq("participant_id", p.id).in("status", ch.allow_late_submission ? ["available", "missed", "skipped"] : ["available"]);
    ...
    if (action === "complete_manual") {
      ...
      const okStatuses = ch.allow_late_submission ? ["available", "started", "missed", "skipped"] : ["available", "started"];
