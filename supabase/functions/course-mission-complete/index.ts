...
import { completeMission, buildStatus, resolveGamificationUserId } from "../_shared/gamification.ts";
...
Deno.serve(async (req) => {
  ...
  try {
    const { userId, email, lessonId, courseId } = await req.json();
    const uid = await resolveGamificationUserId(userId, email);
    if (!uid || !lessonId) return json({ success: false, error: "حساب دانشجو و درس الزامی است" }, 400);
