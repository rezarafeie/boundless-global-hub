...
export async function syncParticipant(ch: Challenge, p: Participant, structure?: { days: any[]; variants: any[] }) {
  ...
  const open = (ch as any).allow_late_submission ? [...OPEN, "missed", "skipped"] : OPEN;
...
export async function processExpiredMissions(
  ...
  for (const row of rows ?? []) {
    ...
    const joinedDay = currentDayNumber(ch, Date.parse(p.joined_at)) || 1;
    const preJoin = row.day_number < joinedDay && !(ch as any).allow_join_after_start && ["available", "skipped"].includes(row.status);
