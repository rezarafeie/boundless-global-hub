-- Create sales_manager role enum and update existing role enum
DO $$ BEGIN
    -- Check if sales_manager already exists in the enum
    IF NOT EXISTS (SELECT 1 FROM pg_enum WHERE enumlabel = 'sales_manager' AND enumtypid = (SELECT oid FROM pg_type WHERE typname = 'academy_user_role')) THEN
        ALTER TYPE academy_user_role ADD VALUE 'sales_manager';
    END IF;
END $$;
