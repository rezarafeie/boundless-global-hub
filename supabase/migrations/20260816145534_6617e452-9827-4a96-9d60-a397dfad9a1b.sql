ALTER TABLE public.webinar_followups
  ...
  ADD COLUMN IF NOT EXISTS min_interval_minutes integer NOT NULL DEFAULT 30,
  ADD COLUMN IF NOT EXISTS do_not_send_after_webinar_start boolean NOT NULL DEFAULT true,
  ADD COLUMN IF NOT EXISTS quiet_hours_start integer,
  ADD COLUMN IF NOT EXISTS quiet_hours_end integer,
  ADD COLUMN IF NOT EXISTS final_lead_minutes integer NOT NULL DEFAULT 15;

CREATE OR REPLACE FUNCTION public.update_webinar_followup_v2(
  ...
  p_quiet_hours_start integer,
  p_quiet_hours_end integer,
  p_final_lead_minutes integer
)
RETURNS webinar_followups
LANGUAGE plpgsql
SECURITY DEFINER
