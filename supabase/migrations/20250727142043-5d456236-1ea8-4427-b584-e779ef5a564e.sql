...
-- Update the assign_courses_to_sales_agent function to be more robust
...
BEGIN
  ...
  LOOP
    INSERT INTO public.sales_agent_courses (sales_agent_id, course_id, created_at, updated_at)
    VALUES (agent_id, course_id, now(), now())
...
DROP POLICY IF EXISTS "Allow sales agent course management" ON public.sales_agent_courses;
CREATE POLICY "Allow sales agent course management" ON public.sales_agent_courses
FOR ALL USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "Allow sales agent management" ON public.sales_agents;
CREATE POLICY "Allow sales agent management" ON public.sales_agents
FOR ALL USING (true) WITH CHECK (true);
