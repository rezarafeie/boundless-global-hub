-- Update the send_message_webhook trigger function to include delete link
...
BEGIN
  ...
  delete_link := 'https://ihhetvwuhqohbfgkqoxw.supabase.co/functions/v1/delete-message-public?messageId=' || NEW.id || '&table=' || TG_TABLE_NAME;
  ...
  SELECT * INTO http_response FROM http_post(
    ...
    concat(
      ...
      '&delete_link=', url_encode(delete_link)
    ),
