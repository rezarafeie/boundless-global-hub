...
-- Create sales_agents table to store sales agent information
...
-- Create sales_agent_courses table to define which courses a sales agent can access
...
-- Create lead_assignments table to track lead assignments
...
-- Function to get sales agent courses
...
-- Function to get available leads for a sales agent
...
-- Function to get lead assignments for a sales agent
...
-- Function to assign lead to sales agent
...
-- Function to assign courses to sales agent
...
-- Function to remove course from sales agent
...
-- Function to check if user has sales agent access to lead
...
-- Add sales_agent role to user_roles if not exists
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM public.user_roles 
    WHERE role_name = 'sales_agent' 
    LIMIT 1
  ) THEN
    -- This is just to ensure the role exists in the system
    -- Individual assignments will be done through the UI
    NULL;
  END IF;
END $$;
