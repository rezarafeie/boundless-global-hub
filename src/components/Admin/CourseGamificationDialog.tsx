        ...
        {loading ? (
          ...
          <div className="space-y-5">
                  ...
                  <CardContent className="space-y-2 p-3">
                    <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
                      <Input
                        ...
                        value={rw.title}
                        onChange={(e) =>
                          setRewards(rewards.map((x, j) => (j === i ? { ...x, title: e.target.value } : x)))
                        }
                      />
                      <Input
                        placeholder="ایموجی"
                        value={rw.emoji}
                        onChange={(e) =>
                          setRewards(rewards.map((x, j) => (j === i ? { ...x, emoji: e.target.value } : x)))
                        }
                      />
                      <Input
                        type="number"
                        ...
                        onChange={(e) =>
                          setRewards(
                            rewards.map((x, j) => (j === i ? { ...x, within_days: Number(e.target.value) } : x)),
                          )
                        }
                      />
                      <Input
                    ...
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <div className="flex shrink-0 items-center gap-2">
                        <Switch
                          ...
                          onCheckedChange={(v) =>
                            setRewards(rewards.map((x, j) => (j === i ? { ...x, is_active: v } : x)))
                          }
                        />
                        <Button size="icon" variant="ghost" onClick={() => removeReward(i)}>
            ...
            <Button className="w-full" onClick={save} disabled={saving}>
              {saving && <Loader2 className="ml-2 h-4 w-4 animate-spin" />} ذخیره تنظیمات
            </Button>
          </div>
        )}
      </DialogContent>
