...
import { MessageCircle, Phone, User, Lock, AtSign, AlertCircle, Check, Loader2, ChevronDown } from 'lucide-react';
...
import { detectCountryCode, formatPhoneWithCountryCode, getCountryCodeOptions, getCountryByCode } from '@/lib/countryCodeUtils';
...
const UnifiedMessengerAuth: React.FC<UnifiedMessengerAuthProps> = ({ onAuthenticated, prefillData, linkingEmail, isAcademyAuth = false }) => {
  ...
  const isPhoneStep = currentStep === 'phone';

  return (
    <Card className="w-full max-w-sm border-0 shadow-none bg-background mx-auto">
      <CardHeader className={`text-center ${isPhoneStep ? 'pb-6 pt-2' : 'pb-8'}`}>
        {!isPhoneStep && (
          <div className="flex justify-center mb-6">
        ...
        <CardTitle className={`${isPhoneStep ? 'text-xl' : 'text-2xl'} font-semibold tracking-tight text-foreground`}>
          {getStepTitle()}
        </CardTitle>
      ...
      <CardContent className={isPhoneStep ? 'px-2' : ''}>
        {isPhoneStep && authMethod === 'telegram' && (
          ...
          <form onSubmit={handlePhoneSubmit} className="space-y-4">
            ...
            >
              ...
              <Input
                ...
                required
                dir="ltr"
                className="flex-1 h-full border-0 rounded-none bg-transparent px-4 text-base focus-visible:ring-0 placeholder:text-muted-foreground/70"
              />
            </div>

            <Button
              type="submit"
              className="w-full h-12 rounded-2xl bg-primary hover:bg-primary/90 text-primary-foreground font-medium text-base shadow-sm"
              disabled={loading}
            >
              {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : 'ادامه'}
            </Button>
              ...
              <>
                <div className="relative py-1">
                  ...
                  <div className="relative flex justify-center text-[11px] uppercase tracking-wider">
                    <span className="bg-background px-3 text-muted-foreground">یا ادامه با</span>
                  </div>
                </div>
                ...
                <div className="flex gap-2.5">
                  {isGoogleAuthEnabled && (
                    <Button
                      ...
                      variant="outline"
                      className="flex-1 h-12 rounded-2xl border-border bg-background hover:bg-muted/60 text-foreground font-medium flex items-center justify-center gap-2.5"
                    >
                      {googleLoading ? (
                  ...
                  <Button
                    ...
                    className="flex-1 h-12 rounded-2xl border-border bg-background hover:bg-muted/60 text-foreground font-medium flex items-center justify-center gap-2.5"
                  >
                    <svg className="w-[18px] h-[18px]" viewBox="0 0 24 24" fill="#229ED9">
                      <path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.473-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.231-.696.064-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.139-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z"/>
                    </svg>
                    Telegram
                  </Button>
                </div>
