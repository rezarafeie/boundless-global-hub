
// Add to existing translations
export const fa = {
  // General translations
  search: "جستجو",
  home: "خانه",
  courses: "دوره ها",
  assessmentCenter: "مرکز ارزیابی",
  blog: "وبلاگ",
  contactUs: "تماس با ما",
  aboutUs: "درباره ما",
  termsOfService: "شرایط خدمات",
  privacyPolicy: "سیاست حفظ حریم خصوصی",
  faq: "سوالات متداول",
  loading: "در حال بارگیری...",
  successMessage: "عملیات با موفقیت انجام شد.",
  errorMessage: "خطایی رخ داده است. لطفا دوباره تلاش کنید.",
  noDataAvailable: "اطلاعاتی در دسترس نیست.",
  
  // Course Archive translations
  allCourses: "همه دوره ها",
  freeCoursesTitle: "دوره های رایگان",
  paidCoursesTitle: "دوره های پولی",
  courseDetails: "جزئیات دوره",
  
  // Course View translations
  courseObjectives: "اهداف دوره",
  whatYouWillLearn: "آنچه خواهید آموخت",
  courseContent: "محتوای دوره",
  instructor: "مدرس",
  enrollNow: "ثبت نام",
  
  // Assessment Center translations
  availableAssessments: "ارزیابی های موجود",
  startAssessment: "شروع ارزیابی",
  startTest: "شروع آزمون",
  
  // Instructor Profile translations
  instructorName: "نام مدرس",
  instructorBio: "بیوگرافی مدرس",
  coursesTaught: "دوره های تدریس شده",

  // Auth related translations
  login: "ورود",
  register: "ثبت نام",
  logout: "خروج",
  email: "ایمیل",
  phone: "شماره تلفن",
  password: "رمز عبور",
  firstName: "نام",
  lastName: "نام خانوادگی",
  continue: "ادامه",
  verify: "تایید",
  back: "بازگشت",
  resendCode: "ارسال مجدد کد",
  codeSent: "کد ارسال شد",
  codeEmailSent: "کد به ایمیل ارسال شد",
  codePhoneSent: "کد به شماره تلفن ارسال شد",
  codeExpires: "انقضای کد در",
  invalidCode: "کد تایید نامعتبر است",
  creating: "در حال ایجاد...",
  sending: "در حال ارسال...",
  loginRequired: "ورود به سیستم لازم است",
  loginSuccess: "ورود موفقیت آمیز",
  logoutSuccess: "خروج موفقیت آمیز",
  accountCreated: "حساب کاربری با موفقیت ایجاد شد",
  profileUpdated: "پروفایل با موفقیت به‌روزرسانی شد",
  courseAccessGranted: "دسترسی به دوره اعطا شد",
  testAccessGranted: "دسترسی به آزمون اعطا شد",
  assistantActivated: "دستیار هوشمند فعال شد",
  error: "خطا",
  success: "موفقیت",
  
  // Dashboard related
  dashboard: "داشبورد",
  profile: "پروفایل",
  myProfile: "پروفایل من",
  personalInformation: "اطلاعات شخصی",
  updateProfileInfo: "به‌روزرسانی اطلاعات شخصی",
  saveChanges: "ذخیره تغییرات",
  saving: "در حال ذخیره...",
  updateProfileError: "خطا در به‌روزرسانی پروفایل",
  
  // Navigation and site sections
  rafiei: "رفیعی",
  academy: "آکادمی",
  support: "پشتیبانی",
  usePassword: "استفاده از رمز عبور",
  useVerificationCode: "استفاده از کد تایید",
  enterEmail: "لطفاً ایمیل را وارد کنید",
  enterPhone: "لطفاً شماره تلفن را وارد کنید",
  emailOrPhonePlaceholder: "example@mail.com | ۰۹۱۲۳۴۵۶۷۸۹",
  fillAllFields: "لطفاً همه فیلدها را پر کنید",
  verificationCodeSent: "کد تایید ارسال شد",
  creatingAccount: "در حال ایجاد حساب کاربری...",
  accountCreatedSuccessfully: "حساب کاربری با موفقیت ایجاد شد!",
  welcomeBack: "خوش آمدید!",
  registrationSuccess: "ثبت نام موفق!",
  enterEmailOrPhone: "ایمیل یا شماره تلفن خود را وارد کنید",
  emailOrPhone: "ایمیل یا تلفن",
  sendVerificationCode: "ارسال کد تایید",
  loginRegister: "ورود / ثبت نام",
  accessCourse: "دسترسی به دوره",
  firstNamePlaceholder: "نام",
  lastNamePlaceholder: "نام خانوادگی",
  
  // AI Assistant related
  aiAssistant: "دستیار هوشمند",
  aiAssistantTitle: "دستیار هوشمند آموزشی",
  aiAssistantName: "دستیار یادگیری",
  aiAssistantDescription: "دریافت کمک شخصی‌سازی شده در مسیر یادگیری",
  aiAssistantLongDescription: "دستیار هوشمند ما راهنمایی شخصی‌سازی شده به شما ارائه می‌دهد، به سؤالات شما پاسخ می‌دهد و به شما کمک می‌کند تا مفاهیم را به طور مؤثر تمرین کنید. در دسترس ۲۴ ساعت در روز برای پشتیبانی از سفر یادگیری شما.",
  aiAssistantFeature1Title: "پشتیبانی آموزشی 24/7",
  aiAssistantFeature2Title: "راهنمایی شخصی‌سازی شده",
  aiAssistantFeature3Title: "تمرین‌های عملی",
  aiAssistantFeature4Title: "یادگیری تعاملی",
  aiAssistantFeature4: "یادگیری از طریق راهنمایی گفتگویی و تعاملی",
  aiAssistantAction: "همین حالا دستیار هوشمند را امتحان کنید",
  startAssistant: "شروع استفاده از دستیار",
  aiFeature1: "کمک فوری",
  aiFeature2: "پیشنهادات هوشمند",
  aiFeature3: "پیگیری پیشرفت",
  aiFeature1Desc: "سوال بپرسید و در هر زمان کمک فوری دریافت کنید",
  aiFeature2Desc: "توصیه‌های شخصی‌سازی شده بر اساس سبک یادگیری شما دریافت کنید",
  aiFeature3Desc: "دانش خود را با سوالات تمرینی تولید شده توسط هوش مصنوعی بیازمایید",
  aiAssistantFeature1Desc: "سوال بپرسید و در هر زمان کمک فوری دریافت کنید",
  aiAssistantFeature2Desc: "توصیه‌های شخصی‌سازی شده بر اساس سبک یادگیری شما دریافت کنید",
  aiAssistantFeature3Desc: "دانش خود را با سوالات تمرینی تولید شده توسط هوش مصنوعی بیازمایید",
  
  // Assessment Center
  personalityTests: "آزمون‌های شخصیت",
  intelligenceTests: "آزمون‌های هوش",
  careerTests: "آزمون‌های شغلی",
  emotionTests: "آزمون‌های احساسی",
  assessmentCenterTitle: "مرکز ارزیابی",
  assessmentCenterDesc: "با ابزارهای ارزیابی حرفه‌ای ما، بیشتر درباره خودتان بدانید",
  
  // Course sections
  trainingCenter: "مرکز آموزش",
  trainingCenterDesc: "مهارت‌های خود را با دوره‌های حرفه‌ای ما ارتقا دهید",
  boundlessProgram: "دوره بی‌کرانگی",
  boundlessProgramDesc: "ذهنیت خود را متحول کنید و به موفقیت‌های نامحدود دست یابید",
  boundlessBenefits: "ذهنیت کارآفرینان موفق را یاد بگیرید",
  boundlessOutcome: "نقشه راهی برای رشد فردی و حرفه‌ای خود ایجاد کنید",
  instagramEssentials: "اصول اینستاگرام",
  instagramEssentialsDesc: "بازاریابی اینستاگرام را برای رشد کسب و کار خود تسلط پیدا کنید",
  instagramBenefits: "یاد بگیرید چگونه محتوای جذابی که به فروش منجر می‌شود تولید کنید",
  instagramOutcome: "یک استراتژی بازاریابی موفق اینستاگرام ایجاد کنید",
  boundlessTaste: "پیش‌نمایش دوره بی‌کرانگی",
  boundlessTasteDesc: "معرفی رایگان به ذهنیت بی‌کرانگی",
  boundlessTasteBenefits: "طعمی از اصول متحول‌کننده ذهنیت را تجربه کنید",
  boundlessTasteOutcome: "مفاهیم اولیه را برای دیدن نتایج فوری اعمال کنید",
  passiveIncomeAI: "درآمد غیرفعال با هوش مصنوعی",
  passiveIncomeAIDesc: "از ابزارهای هوش مصنوعی برای ایجاد جریان‌های درآمدی غیرفعال استفاده کنید",
  passiveIncomeAIBenefits: "جدیدترین ابزارهای هوش مصنوعی برای تولید محتوا را کشف کنید",
  passiveIncomeAIOutcome: "اولین سیستم درآمد خودکار خود را راه‌اندازی کنید",
  magazine: "مجله",
  magazineDesc: "آخرین بینش‌ها و دانش برای کارآفرینان مدرن",
  readMore: "بیشتر بخوانید",
  learnMore: "بیشتر بدانید",
  slogan: "پتانسیل خود را آزاد کنید",
  tagline: "ذهنیت و مهارت‌های خود را با دوره‌های توسعه حرفه‌ای ما متحول کنید",
  callToAction: "شروع یادگیری",
  beginner: "مبتدی",
  intermediate: "متوسط",
  advanced: "پیشرفته",
  
  // Additional keys for components that were missing
  activeStatus: "فعال",
  upcomingStatus: "به زودی",
  completedStatus: "تکمیل شده",
  startCourse: "شروع دوره",
  startFreeCourse: "شروع دوره رایگان",
  websiteName: "آکادمی رفیعی",
  footerDesc: "توانمندسازی ذهن‌ها از طریق آموزش حرفه‌ای و تجربه‌های یادگیری تحول‌آفرین.",
  myAccount: "حساب کاربری من",
  metaverseEmpire: "امپراتوری متاورس",
  wealthCourse: "ثروت‌سازی",
  footerCopyright: "© 2023 آکادمی رفیعی. تمام حقوق محفوظ است.",
  terms: "قوانین استفاده",
  privacy: "حریم خصوصی",
  freeCoursesSubtitle: "مهارت‌های ارزشمند را بدون هزینه یاد بگیرید",
  coursesTitle: "دوره‌های رایگان موجود",
  coursesSubtitle: "سفر یادگیری خود را امروز آغاز کنید",
  changeProject: "پروژه تغییر",
  changeProjectDesc: "یاد بگیرید چگونه تغییرات را به طور موثر مدیریت کنید",
  changeProjectBenefits: "اصول مدیریت تغییر را به خوبی فرا بگیرید",
  changeProjectOutcome: "تغییرات را با موفقیت در سازمان خود پیاده‌سازی کنید",
  americanBusiness: "تجارت آمریکایی",
  americanBusinessDesc: "با شیوه‌های تجاری آمریکایی آشنا شوید",
  americanBusinessBenefits: "درباره فرهنگ تجاری و آداب معاشرت آمریکایی بیاموزید",
  americanBusinessOutcome: "با اعتماد به نفس در محیط‌های تجاری آمریکایی فعالیت کنید",
  startLearning: "شروع یادگیری",

  // Course details
  courseCurriculum: "سرفصل دوره",
  aboutInstructor: "درباره مدرس",
  studentFeedback: "بازخورد دانشجویان",
  overview: "مرور کلی",
  lessons: "درس‌ها",
  quizzes: "آزمون‌ها",
  announcements: "اعلانات",
  reviews: "نظرات",
  noReviewsYet: "هنوز نظری ثبت نشده است.",
  addReview: "افزودن نظر",
  yourReview: "نظر شما",
  submitReview: "ثبت نظر",
  editReview: "ویرایش نظر",
  deleteReview: "حذف نظر",
  confirmDeleteReview: "آیا مطمئن هستید که می‌خواهید نظر خود را حذف کنید؟",
  cancel: "انصراف",
  confirm: "تایید",
  description: "توضیحات",
  requirements: "پیش‌نیازها",
  targetAudience: "مخاطبان هدف",
  addToCart: "افزودن به سبد خرید",
  buyNow: "خرید فوری",
  congratulations: "تبریک!",
  courseComplete: "شما این دوره را تکمیل کرده‌اید.",
  continueToNext: "ادامه به بعدی",
  wellDone: "آفرین!",
  quizComplete: "شما این آزمون را تکمیل کرده‌اید.",
  yourScore: "امتیاز شما:",
  retakeQuiz: "تکرار آزمون",
  courseIncludes: "این دوره شامل",
  hoursOfVideo: "ساعت ویدیو",
  articles: "مقاله",
  downloadableResources: "منابع قابل دانلود",
  fullLifetimeAccess: "دسترسی مادام‌العمر",
  certificateOfCompletion: "گواهی تکمیل دوره",
  frequentlyAskedQuestions: "سوالات متداول",
  studentReviews: "نظرات دانشجویان",
  relatedCourses: "دوره‌های مرتبط",
  share: "اشتراک‌گذاری",
  viewCourse: "مشاهده دوره",
  lesson: "درس",
  quiz: "آزمون",
  complete: "تکمیل شده",
  next: "بعدی",
  previous: "قبلی",
  resources: "منابع",
  download: "دانلود",
  noLessons: "هیچ درسی در این ماژول وجود ندارد.",
  noQuizzes: "هیچ آزمونی در این ماژول وجود ندارد.",
  noAnnouncements: "هنوز اعلانی وجود ندارد.",
  searchCourses: "جستجوی دوره‌ها",

  // FreeCourseStart component translations
  completed: "تکمیل شده",
  done: "انجام شد",
  freeCourseWelcome: "به دوره رایگان خوش آمدید! مراحل زیر را برای شروع تکمیل کنید.",
  courseFiles: "فایل‌های دوره",
  courseFilesDescription: "به تمام مواد آموزشی و منابع دوره دسترسی پیدا کنید.",
  courseSlides: "اسلایدهای دوره",
  worksheets: "کاربرگ‌ها",
  activateAccess: "فعال‌سازی دسترسی",
  activateAccessDescription: "این مراحل را برای دریافت دسترسی کامل به دوره خود تکمیل کنید",
  joinChannel: "عضویت در کانال خصوصی تلگرام",
  joinChannelDescription: "دریافت به‌روزرسانی‌ها و ارتباط با سایر دانشجویان",
  joinNow: "همین حالا عضو شوید",
  activateSupport: "فعال‌سازی پشتیبانی دوره",
  activateSupportDescription: "دسترسی به پشتیبانی مدرس و پرسش و پاسخ",
  activate: "فعال‌سازی",
  smartAssistant: "راه‌اندازی دستیار هوشمند",
  smartAssistantDescription: "دریافت کمک هوشمند برای سؤالات دوره",
  launch: "راه‌اندازی",

  // FreeCourseView component translations
  freeCourseAccess: "دسترسی دوره رایگان",
  freeCourseAccessInstructions: "معرفی دوره و نمونه محتوا را در زیر تماشا کنید.",
  videoPlayerPlaceholder: "ویدیو در اینجا بارگذاری می‌شود",
  youtubeEmbed: "ویدیوی یوتیوب در اینجا جاسازی می‌شود",
  upgradeToPremium: "ارتقا به نسخه حرفه‌ای",
  upgradeToPremiumDescription: "دسترسی کامل به تمام مطالب دوره و پشتیبانی مدرس را دریافت کنید",
  viewPaidCourses: "مشاهده دوره‌های حرفه‌ای",
  
  // PaidCourseStart component translations
  paidCourseWelcome: "به دوره حرفه‌ای خوش آمدید! مراحل زیر را برای شروع تکمیل کنید.",
  rafeiPlayer: "پخش‌کننده رفیعی",
  playerActivationDescription: "دسترسی به ویدیوهای دوره از طریق پخش‌کننده رفیعی.",
  activationCode: "کد فعال‌سازی",
  downloadDesktopPlayer: "دانلود برای دسکتاپ",
  downloadMobilePlayer: "دانلود برای موبایل",
  openWebPlayer: "باز کردن پخش‌کننده وب",

  // PaidCourseView component translations
  paidCourseAccessInstructions: "به تمام محتوای دوره حرفه‌ای خود در زیر دسترسی پیدا کنید.",
  courseMaterials: "مواد آموزشی دوره",
  courseMaterialsDescription: "دسترسی به تمام مواد آموزشی و منابع دوره حرفه‌ای.",
  communityAccess: "دسترسی به انجمن",
  communityAccessDescription: "با سایر دانشجویان ارتباط برقرار کنید و از مدرسان پشتیبانی دریافت کنید.",
  
  // CourseArchive component translations
  wealthCourseDesc: "ثروت پایدار با استراتژی‌های مالی اثبات شده بسازید",
  wealthBenefits: "یاد بگیرید چگونه جریان‌های درآمدی متعدد ایجاد کنید",
  wealthOutcome: "به استقلال و امنیت مالی دست یابید",
  metaverseEmpireDesc: "تسلط بر فناوری‌ها و فرصت‌های متاورس",
  metaverseBenefits: "کشف مدل‌های کسب و کار نوظهور در متاورس",
  metaverseOutcome: "خود را به عنوان رهبر در اقتصادهای مجازی قرار دهید",
  businessCourses: "دوره‌های کسب و کار",
  selfDevelopmentCourses: "توسعه فردی",
  currentlyRunning: "دوره‌های در حال اجرا",
  currentlyRunningDesc: "به این دوره‌ها که در حال پیشرفت هستند بپیوندید.",
  upcomingCourses: "دوره‌های آینده",
  upcomingCoursesDesc: "اولین نفری باشید که برای این دوره‌های هیجان‌انگیز آینده ثبت نام می‌کند.",
  pastCourses: "دوره‌های گذشته",
  pastCoursesDesc: "به آرشیو دوره‌های برگزار شده قبلی دسترسی پیدا کنید.",

  // BoundlessLanding translations
  boundlessLandingTitle: "شروع بدون مرز: پتانسیل کامل خود را آزاد کنید",
  boundlessLandingSubtitle: "ذهنیت خود را برای موفقیت متحول کنید",
  boundlessLandingHeroDesc: "برنامه جامعی که برای شکستن محدودیت‌هایتان و دستیابی به نتایج خارق‌العاده در کسب‌وکار و زندگی طراحی شده است.",
  boundlessLandingOriginalPrice: "قیمت اصلی: ۲۰۰ هزار تومان",
  boundlessLandingSpecialOffer: "تنها ۲۷ هزار تومان",
  boundlessLandingInclusive: "همه چیز شامل",
  boundlessLandingStartCourse: "همین حالا ثبت نام کنید",
  boundlessLandingJoinLater: "برای بعداً ذخیره کنید",
  boundlessLandingCountdown: "پیشنهاد ویژه به پایان می‌رسد:",
  boundlessLandingAboutTitle: "درباره این برنامه",
  boundlessLandingWhoFor: "این برنامه برای چه کسانی است؟",
  boundlessLandingSolutions: "چه چیزی یاد خواهید گرفت",
  boundlessLandingResultsTitle: "نتایجی که می‌توانید انتظار داشته باشید",
  boundlessLandingWhatLearn: "آنچه تسلط خواهید یافت",
  boundlessLandingSkill1: "تفکر بدون مرز",
  boundlessLandingSkill2: "تسلط بر دراپ‌شیپینگ",
  boundlessLandingSkill3: "ارائه خدمات",
  boundlessLandingSkill4: "محصولات دیجیتال",
  boundlessLandingSkill5: "سیستم‌های مقیاس‌پذیر",
  boundlessLandingBonusesTitle: "بونوس‌های انحصاری",
  boundlessLandingBonus1: "دوره درآمد با هوش مصنوعی",
  boundlessLandingBonus2: "ارزیابی شغلی",
  boundlessLandingBonus3: "نقشه راه موفقیت",
  boundlessLandingTestimonialsTitle: "داستان‌های موفقیت دانشجویان",
  boundlessLandingFAQTitle: "سوالات متداول",
  boundlessLandingFitTitle: "آیا این مناسب شماست؟",
  boundlessLandingFitStudent: "دانشجویان",
  boundlessLandingFitFreelancer: "فریلنسرها",
  boundlessLandingFitRemote: "کارگران راه دور",
  boundlessLandingFitEntrepreneur: "کارآفرینان",
  boundlessLandingFitMigrate: "مهاجران آینده",
  boundlessLandingStickyOffer: "پیشنهاد ویژه: تنها ۲۷ هزار تومان",
  
  // BoundlessLanding FAQ translations
  boundlessLandingFAQ1Q: "چقدر به دوره دسترسی خواهم داشت؟",
  boundlessLandingFAQ1A: "شما دسترسی مادام‌العمر به تمام مطالب دوره و به‌روزرسانی‌ها خواهید داشت.",
  boundlessLandingFAQ2Q: "آیا سیاست بازگشت وجه وجود دارد؟",
  boundlessLandingFAQ2A: "بله، ما ضمانت بازگشت وجه ۳۰ روزه ارائه می‌دهیم اگر راضی نباشید.",
  boundlessLandingFAQ3Q: "چگونه این برنامه با سایر برنامه‌ها متفاوت است؟",
  boundlessLandingFAQ3A: "بی‌کرانگی ابزارهای عملی را با تحول ذهنیت برای نتایج ماندگار ترکیب می‌کند.",
  boundlessLandingFAQ4Q: "آیا نیاز به تجربه قبلی دارم؟",
  boundlessLandingFAQ4A: "هیچ تجربه قبلی لازم نیست. برنامه برای همه سطوح طراحی شده است."
};
